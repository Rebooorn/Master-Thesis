...
class simple_data_provider(BaseDataProvider):
    ...
    def _next_data(self):
        ...
        if self.nbatch == 1:
            ...
            img_orig = cv2.imread(os.path.join(TRAINING_PATH, 'Origin_img', img_names[0]))
            ...
            # a fake 1st label
            label = cv2.imread(os.path.join(TRAINING_PATH, 'Label_Class_1', img_names[0]), 0)
            label = np.reshape(label, (label.shape[0], label.shape[1], 1))
            for i in range(1, self.nclass):
                cls = cv2.imread(os.path.join(TRAINING_PATH, 'Label_Class_'+str(i), img_names[0]), 0)
...
if __name__ == '__main__':
    ...
    generator = simple_data_provider(batch_num=1, nclass=2, channel='full')

    for _ in range(5):
        input_tensor, label_tensor = generator(1)
        shape = input_tensor.shape
        input_tensor = np.reshape(input_tensor, [shape[1], shape[2], shape[3]])
        label_tensor = np.reshape(label_tensor, [shape[1], shape[2], 2])
